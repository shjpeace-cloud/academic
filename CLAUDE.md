...
- **Honors and Awards** (2026-09-17 신설, 사용자 확정 형식): 인천대 총장표창. **웹 CV** = Selected Publications 아래(Contact 위), **수여 연도별 4줄**(연도 칸 2025·2025·2023·2022, 본문에 "(학술연구상, for 2024)"처럼 대상 연도). **PDF** = 맨 아래(Teaching 다음), **상 기준 2줄 + 괄호 대상 연도** — "Research Excellence Award, Incheon National University (2021, 2022, 2024)" / "Excellent Teaching Award, Incheon National University (Fall 2024)". 연도를 연도 열로 옮긴 판(4751050)은 사용자가 "이상함"으로 기각. 원자료는 학교 포상사항 엑셀(루트, `/*.xlsx`로 gitignore — 내부 문서번호 포함이라 공개 금지). 새 수상 시 `cv.html`과 `build_cv.py`의 `AWARDS` **둘 다** 수정 후 PDF 재생성
